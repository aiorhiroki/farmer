...
def cce_dice_loss(beta=1, class_weights=None, class_indexes=None, **kwargs):
    loss = categorical_crossentropy(class_weights=class_weights, class_indexes=class_indexes) \
            + dice_loss(beta=beta, class_weights=class_weights, class_indexes=class_indexes)
    return loss

def cce_jaccard_loss(class_weights=None, class_indexes=None, per_image=False, **kwargs):
    loss = categorical_crossentropy(class_weights=class_weights, class_indexes=class_indexes) \
            + jaccard_loss(class_weights=class_weights, class_indexes=class_indexes, per_image=per_image)
    return loss

def categorical_focal_dice_loss(alpha=0.25, beta=1, gamma=2., class_weights=None, class_indexes=None, **kwargs):
    loss = categorical_focal_loss(alpha=alpha, gamma=gamma, class_indexes=class_indexes) \
            + dice_loss(beta=beta, class_weights=class_weights, class_indexes=class_indexes)
    return loss
